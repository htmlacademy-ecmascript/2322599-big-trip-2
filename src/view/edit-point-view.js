...
function createEditPointTemplate(state, destination, offers, destinations) {
  const { dateFrom, dateTo, selectedOffers, type, basePrice, isDisabled, isSaving, isDeleting } = state;
...
export default class EditPointView extends AbstractStatefulView {
  ...
  get template() {
    return createEditPointTemplate(this._state, this.#destination, this.#offers, this.#pointsModel.destinations);
  }
  ...
  #validateForm() {
    ...
    const isValidDestination = this.#pointsModel.destinations.some((dest) => dest.name === destinationInput.value
    );
  ...
  #destinationChangeHandler = (evt) => {
    ...
    const newDestination = this.#pointsModel.destinations.find((dest) => dest.name === selectedDestinationName
    );
